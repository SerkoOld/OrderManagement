...
namespace Order.Management.Services
{
    ...
    public class MarkdownPrinter : IPrinter
    {
        ...
        {
            return
                $"Name: {order.Name} Address: {order.Address} Due Date: {order.DueDate:d MMM yyyy} Order #: {order.OrderNumber}";
        }

        private static string PrintTable(IEnumerable<(ToyShape, int)> tuples, string kLabel,
            string vLabel)
        {
            ...
            table.AppendLine();
            table.AppendLine(
                $"| {new string('-', kLabel.Length)} | {new string('-', vLabel.Length)} |");
        ...
        private static string PrintTable<TR, TC>(Matrix<TR, TC, int> matrix)
        {
            ...
            {
                table.AppendFormat("| {0} | {1} |", row,
                    string.Join(" | ", matrix.Columns.Select(c => matrix[row, c])));
                table.AppendLine();
            }
