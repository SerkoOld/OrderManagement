...
namespace Order.Management.Services
{
    ...
    {
        public static Invoice GenerateInvoice(OrderInfo order, IPricing pricing)
        {
            decimal total = 0;
            ...
            foreach (var item in order.Items)
            {
                ...
                var surchargeItem = surcharges.GetValueOrDefault(item.Color.ToString(),
                    new InvoiceItem()
                    {
        ...
        {
            var table = new Matrix<ToyShape, ToyColor, int>(rows: CommonUtils.GetAllShapes(),
                columns: CommonUtils.GetAllColors());
            foreach (var item in order.Items)
            {
